...
{
    public partial class U_ShiftManager : DevExpress.XtraEditors.XtraUserControl
    {
        CTWH.Common.MSSQL.WMSAccess _WMSAccess;

        public U_ShiftManager()
        {
            InitializeComponent();
        }
        ...
        private void U_ShiftManager_Load(object sender, EventArgs e)
        {
            _WMSAccess = Utils.WMSSqlAccess;
            _WMSAccess.SqlStateChange += new CTWH.Common.MSSQL.WMSAccess.SqlStateEventHandler(access_SqlStateChange);

            WMSDS ids = this._WMSAccess.Select_T_Shift("");
            this.gridShift.DataSource = ids.T_Shift;
