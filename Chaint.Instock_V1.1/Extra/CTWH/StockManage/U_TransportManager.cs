...
{
    public partial class U_TransportManager : DevExpress.XtraEditors.XtraUserControl
    {
        CTWH.Common.MSSQL.WMSAccess _WMSAccess;

        public U_TransportManager()
        {
            InitializeComponent();
        }
        ...
        private void U_TransportManager_Load(object sender, EventArgs e)
        {
            _WMSAccess = Utils.WMSSqlAccess;
            _WMSAccess.SqlStateChange += new CTWH.Common.MSSQL.WMSAccess.SqlStateEventHandler(access_SqlStateChange);

            //WMSDS ids = this._WMSAccess.Select_T_BusinessType("","");
            //this.grid_Tran.DataSource = ids.t_tr;
