...
{
    public partial class U_WarehouseManager : DevExpress.XtraEditors.XtraUserControl
    {
        CTWH.Common.MSSQL.WMSAccess _WMSAccess;

        public U_WarehouseManager()
        {
            InitializeComponent();
        }
        ...
        private void U_WarehouseManager_Load(object sender, EventArgs e)
        {
            _WMSAccess = Utils.WMSSqlAccess;
            _WMSAccess.SqlStateChange += new CTWH.Common.MSSQL.WMSAccess.SqlStateEventHandler(access_SqlStateChange);

            InterfaceDS ids = this._WMSAccess.Select_CT_CKZD("");
            this.grid_WH.DataSource = ids.CT_CKZD;
