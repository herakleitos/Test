...
namespace Chaint.Common.Devices.LED
{
    public class LED_TY
    {
        ...
        private LEDDEV m_LEDHandler = new LEDDEV();
        ...
        public bool ConnectDevice()
        {
            ...
            else if (m_ParamConnect is Param_LED_Ethernet)
            {
                ...
                //iRet = LedDll.ConnectLED_TCPIP(ref cIP[0], (ushort)x, 3000, 2500, ref Led);	//连接超时3秒,帧数据接受超时2.5秒. 网络忙或使用互连网,此两值要加大!!!	
            }
            ...
            // byte[] cIP = new byte[4] { 172, 17, 253, 11 };

            //int iRet = TYLedDll.ConnectLED_TCPIP(ref cIP[0], 8800, 3000, 2500, ref m_LEDHandler);	//连接超时3秒,帧数据接受超时2.5秒. 网络忙或使用互连网,此两值要加大!!!	
            //if (iRet == 0)
            //{ return true; }
            //else
            //    return false;
        ...
        public bool WriteDataDynamic(LED_FontStyle fontStyle, IList<LED_TextStyle> txtStyles, LED_ActionSytle actionStyle)
        {
            bool retVal = false;
            int iRet = 0;
            ...
            try
            {
                ...
                if (blnSucc)
                {
                    iRet = TYLedDll.CreateNewPlayFile(m_LEDHandler.iID);       //新建LED播放文件
                    if (iRet != (int)RetVal.ERR_OK)
                    {
                        TYLedDll.CloseLEDConnect(m_LEDHandler.iID);
                        return false;
                    }
                    ////插入节目
                    TYLedDll.InsertProgram(m_LEDHandler.iID, 1, false, 0);
                    ////插入窗口
                    TYLedDll.InsertWindow(m_LEDHandler.iID, 1, 1, (int)WndType.FILE, 1, 1, 128, 64, (int)WndEdge.NONE, 0, false);
                    //清内存映象内容
                    //TYLedDll.Image_Clear(m_LEDHandler.iID);
                    for (int i = 0; i < txtStyles.Count; i++)
                    {
                        TYLedDll.InsertTextItemW(m_LEDHandler.iID, 1, 1, i + 1,
                                                  txtStyles[i].DisplayText,
                                                  (byte)actionStyle.InMode,
                                                  (byte)actionStyle.InSpeed,
                                                  actionStyle.StayTime,
                                                  (byte)actionStyle.OutMode, (byte)actionStyle.OutSpeed);

                        TYLedDll.SetTextItemFormatW(m_LEDHandler.iID, 1, 1, i + 1,
                            (int)txtStyles[i].TextColor, 0, 0, 0, 1, false, fontStyle.FontName,
                            fontStyle.FontWidth, fontStyle.FontHeight, false, false);
                    }
                    //送画面到LED屏. 进入:立即显示,退出:无动作,中速。函数等待发送结束。
                    iRet = TYLedDll.SendToLED(m_LEDHandler.iID, (byte)PlayCon.ALWAYS, 0, true);
                    if (iRet != (int)RetVal.ERR_OK)
                    {
                        retVal = false;
                    }
