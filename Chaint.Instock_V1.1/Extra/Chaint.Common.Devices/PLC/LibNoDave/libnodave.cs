        ...
        */
        ...
        public class daveInterface : pseudoPointer
        {
            ...
            */
            ...
            public daveInterface(daveOSserialType fd,
                string name,
                int localMPI,
                DaveProtocol useProto,
                DaveSpeed speed)
            {
                pointer = daveNewInterface(fd, name, localMPI, useProto.GetHashCode(), speed.GetHashCode());
            }
        ...
        public class daveConnection : pseudoPointer
        {
            ...
            */
            ...
            public int readBytes(DaveArea area, int DBnumber, int start, int len, byte[] buffer)
            {
                return daveReadBytes(pointer, area.GetHashCode(), DBnumber, start, len, buffer);
            }

            [DllImport("libnodave.dll"/*, PreserveSig=false */ )]
            protected static extern int daveReadManyBytes(IntPtr dc, int area, int DBnumber, int start, int len, byte[] buffer);
            public int readManyBytes(DaveArea area, int DBnumber, int start, int len, byte[] buffer)
            {
                return daveReadManyBytes(pointer, area.GetHashCode(), DBnumber, start, len, buffer);
            }

            [DllImport("libnodave.dll"/*, PreserveSig=false */ )]
            protected static extern int daveReadBits(IntPtr dc, int area, int DBnumber, int start, int len, byte[] buffer);
            public int readBits(DaveArea area, int DBnumber, int start, int len, byte[] buffer)
            {
                return daveReadBits(pointer, area.GetHashCode(), DBnumber, start, len, buffer);
            }

            [DllImport("libnodave.dll"/*, PreserveSig=false */ )]
            protected static extern int daveWriteBytes(IntPtr dc, int area, int DBnumber, int start, int len, byte[] buffer);
            public int writeBytes(DaveArea area, int DBnumber, int start, int len, byte[] buffer)
            {
                return daveWriteBytes(pointer, area.GetHashCode(), DBnumber, start, len, buffer);
            }

            [DllImport("libnodave.dll"/*, PreserveSig=false */ )]
            protected static extern int daveWriteManyBytes(IntPtr dc, int area, int DBnumber, int start, int len, byte[] buffer);
            public int writeManyBytes(DaveArea area, int DBnumber, int start, int len, byte[] buffer)
            {
                return daveWriteManyBytes(pointer, area.GetHashCode(), DBnumber, start, len, buffer);
            }

            [DllImport("libnodave.dll"/*, PreserveSig=false */ )]
            protected static extern int daveWriteBits(IntPtr dc, int area, int DBnumber, int start, int len, byte[] buffer);
            public int writeBits(DaveArea area, int DBnumber, int start, int len, byte[] buffer)
            {
                return daveWriteBits(pointer, area.GetHashCode(), DBnumber, start, len, buffer);
            }
