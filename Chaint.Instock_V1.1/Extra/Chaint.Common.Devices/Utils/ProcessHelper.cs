...
namespace Chaint.Common.Devices.Utils
{
    ...
    {
        /// <summary>
        /// 启动一个进程 此进程启动时，一般在程序的UI线程中调用此函数,而不是在其他线程中
        /// 
        /// 
        /// </summary>
        /// <param name="strProcessFilePath">进程的全路径</param>
        public static void StartProcessAsNoUserWrapper(string strProcessFilePath)
        {
            Process proess = new Process();
            proess.StartInfo.FileName = strProcessFilePath;
            proess.StartInfo.UseShellExecute = true;
            proess.Start();
            proess.WaitForExit(10);         //等待10毫秒后关闭
            proess.Close();
            proess.Dispose();
        }

        /// <summary>
        /// 启动一个进程 此进程启动时，一般在程序的另外一个线程中启动,可以用于Windows服务程序中启动一个UI应用程序
        /// 注意与上述StartProcessAsNoUserWrapper 进行区分
        /// </summary>
        /// <param name="strProcessFilePath">进程的全路径</param>
        public static void StartProcessAsUserWrapper(string strProcessFilePath)
        {
                    ...
                    if (WTS_CONNECTSTATE_CLASS.WTSActive == tSessionInfo.State)
                    {
                        ...
                        if (WTSQueryUserToken(tSessionInfo.SessionID, out hToken))
                        {
                            ...
                            bool ChildProcStarted = CreateProcessAsUser(
                                ...
                                strProcessFilePath,      // Name of the process to be started.
        ...
        /// 杀死进程
        /// </summary>
        /// <param name="strProcessName">不带扩展名称的进程名称</param>
        public static void StopProcess(string strProcessName)
        {
            Process proess = new Process();
            proess.StartInfo.FileName = strProcessName;
            proess.StartInfo.UseShellExecute = true;
            proess.Kill();
            proess.WaitForExit(10);         //等待10毫秒后关闭
            proess.Close();
        ...
        /// 判断指定进程是否已经运行,如运行返回真
        /// </summary>
        /// <param name="strProcessName">不带扩展名的进程名称</param>
        /// <returns></returns>
        public static bool IsExistsProcessName(string strProcessName)
        {
            return (System.Diagnostics.Process.GetProcessesByName(strProcessName).Length > 0);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static ArrayList GetAllProcessNames()
        {
            ArrayList lstProcessNames = new ArrayList();

            Process[] processes = System.Diagnostics.Process.GetProcesses();

            foreach (Process item in processes)
            {
                lstProcessNames.Add(item.ProcessName);
            }

            return lstProcessNames;
        }
