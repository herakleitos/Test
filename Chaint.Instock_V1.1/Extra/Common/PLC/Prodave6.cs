...
namespace CTWH.Common.PLC
{
    public class Prodave6
    {
        ...
        public enum DataType : byte//PLC数据类型
        {
            BYTE = 0x02,
            WORD = 0x04,
            DWORD = 0x06,
        ...
        public extern static int db_read_ex6(UInt16 BlkNr, DataType DType, UInt16 StartNr, ref UInt32 pAmount, UInt32 BufLen,
            byte[] pBuffer, ref UInt32 pDatLen);

        [DllImport("Prodave6.dll")]//PLC db区写入操作
        //参数：data block号、要写入的数据类型、起始地址号、需要写入类型的数量、缓冲区长度（字节为单位）、缓冲区
        public extern static int db_write_ex6(UInt16 BlkNr, DataType Type, UInt16 StartNr, ref UInt32 pAmount, UInt32 BufLen,
            byte[] pBuffer);
