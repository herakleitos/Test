...
namespace CTWH.Common.Serialization
{
	/// <summary>
	...
	public sealed class SerializationWriter: BinaryWriter
	{
		...
		public void WriteOptimized(string value)
		{
			...
			else
			{
				...
				int stringIndex;
				bool isNew = stringLookup.Add(value, out stringIndex);
				...
				{
#if DEBUG
					long currentPosition = OutStream.CanSeek ? OutStream.Position : 0;
#endif
					base.Write(value);
#if DEBUG
		...
		{
			int result = BaseStream.CanSeek ? (int) BaseStream.Position - startPosition : 0;
		...
		public byte[] ToArray()
		{
			...
			int length = (int) memoryStream.Position - startPosition;
		...
		public void WriteTokenizedObject(object value, bool recreateFromType)
		{
			...
			{
				int newToken = objectTokenLookup.Count;
		...
		{
            string[] strings = (string[])typeof(UniqueStringList).GetField("stringList", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(stringLookup);
			int result = 0;

			for(int i = 0; i < stringLookup.Count; i++)
			{
		...
		sealed class UniqueStringList
		{
			...
			static readonly int[] PrimeNumberList = new int[] 
				{
