...
def create_random_context(dialog,rng,minimum_context_length=2,max_context_length=20):
    ...
    # create string
    while True:
        context = dialog_turns_to_string(dialog[1:context_turns])
        context_array = context.split(' ')
        if len(context_array)>160:
            if context_turns-2>0:
                context_turns-=2
            else:
                context_turns=-1
    ...
    if context_turns<=0:
        return '', 0
    return dialog_turns_to_string(dialog[1:context_turns]),context_turns
...
def train_cmd(args):
    ...
    dialog_paths =[]
    row_data_path = r'E:\Demo Code\Tensorflow\ubuntu-ranking-dataset-creator-master\rawdata\train'
